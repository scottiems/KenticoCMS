...
links.Timeline.prototype.repaintFrame = function() {
    ...
    if (!dom.content) {
    	...
    	/* Brandon 7/21/2013 (BUG 7149) - remove dblClick listener to prevent the user from adding a new event 
			I'm sure we will want to add this back at some point to allow the creation of new events
...
links.Timeline.prototype.repaintDeleteButton = function () {
	//Added this to override the delete icon, it would not show.
	//Need to add an "option" to disable it
}

links.Timeline.prototype.repaintDeleteButton_orig = function () {
    var timeline = this,
        dom = this.dom,
        frame = dom.items.frame;
    ...
    if (!navBar) {
        ...
        if (showNavigation || showButtonNew) {
            ...
            if (options.groupsOnRight) {
            	...
            	navBar.style.right = '5px'; //Brandon change to reposition navBar 
            }
            else {
            	//navBar.style.right = '10px';
            	navBar.style.left = '5px'; //Brandon change to reposition navBar 
            }
            if (options.axisOnTop) {
            	//navBar.style.bottom = '10px';
            	navBar.style.top = '5px'; //Brandon change to reposition navBar 
            }
            else {
            	//navBar.style.top = '10px';
            	navBar.style.bottom = '5px'; //Brandon change to reposition navBar 
            }
            dom.navBar = navBar;
...
function getDocIDForGroupName(dataObj, groupName) {

	var retVal = "";
	for (var i = 0; i < dataObj.length; i++) {
		if (dataObj[i].group == groupName) {
			return dataObj[i].nodeLevel + " " + dataObj[i].aliasPath;
		}
	}
	return retVal;
}
...
links.Timeline.prototype.getGroup = function (groupName) {
    ...
    if (groupIndex == undefined && groupName != undefined) { // not null or undefined
        groupObj = {
            ...
            'labelTop': 0,
            'lineTop': 0,
            'aliasPath': getDocIDForGroupName(this.data, groupName)
